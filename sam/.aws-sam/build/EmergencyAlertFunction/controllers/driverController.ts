...
export const handler = async (
  ...
  try {
    ...
    if (httpMethod === "POST" && pathParameters?.proxy === "create") {
      ...
      if (
        ...
        return buildResponse(400, {
          error:
            "userId, name, phone, vehicleType, vehicleNumber, licenseNumber, licenseExpiry, and aadharNumber are required",
        });
      }
    ...
    if (httpMethod === "GET" && pathParameters?.proxy === "available-rides") {
      ...
      const rides = await DriverService.getAvailableRidesForDriver(driverId);
    ...
    if (
      ...
      const result = await DriverService.acceptRide(pathParameters.id, rideId);
    ...
    if (
      ...
      const result = await DriverService.rejectRide(pathParameters.id, rideId);
      return buildResponse(200, result);
    }

    /* ---------------- GET /drivers/{id} ---------------- */
    if (httpMethod === "GET" && pathParameters?.id) {
      const driver = await DriverService.getDriverById(pathParameters.id);
      if (!driver) {
